...
export class StellarService implements OnModuleInit {
  ...
  private paginationCache = new Map<string, CachedPaginationData>();
  ...
  /**
   * Fetches paginated recent transactions for a given Stellar public key.
   * Implements cursor-based pagination and result caching.
   *
   * @param publicKey - The Stellar G... public key of the account
   * @param limit     - Maximum number of transactions per page (default 10)
   * @param cursor    - Cursor for pagination (hash of last transaction to continue from)
   * @returns         Paginated response with records, next cursor, and hasMore flag
   */
  async getRecentTransactions(
    ...
    const cacheKey = `${publicKey}:${cursor || 'start'}`;

    // Check cache first (if recent)
    if (this.paginationCache.has(cacheKey)) {
    ...
    try {
      return await this.rpcClient.executeWithRetry(async (client) => {
        ...
        let query = horizonServer
          ...
          .limit(Math.min(limit, 200)) // Horizon limit is 200
          .order('desc');
        ...
        const paginationResult: PaginationCursor = {
          records: results,
          nextCursor,
          hasMore,
        };
  ...
  private cleanupCache(): void {
    ...
    for (const [key, data] of this.paginationCache.entries()) {
      if (now - data.timestamp > this.CACHE_TTL) {
        expired.push(key);
      }
    }

    expired.forEach((key) => this.paginationCache.delete(key));
    if (expired.length > 0) {
